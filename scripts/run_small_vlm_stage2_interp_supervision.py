...
def train_stage2(args, run_dir: Path, train, val, *, mode: str) -> dict:
    detach = mode == "heima_baseline"
    if mode not in {"heima_baseline", "ours_interp_supervision"}:
    ...
    attr = grad_a_from_interp(args, model_a, processor, tokenizer_a, tokenizer_b, decoders, projectors, base.batch_rows(train, args.batch_size, 0), detach)
    if mode == "heima_baseline" and attr["grad_A_from_interp"] != 0.0:
        raise RuntimeError("Heima baseline interpreter loss returned gradient to A")
    if mode == "ours_interp_supervision" and not (attr["grad_A_from_interp"] > 0 and attr["finite"]):
        raise RuntimeError("Ours interpreter loss did not reach A")
    logs = []
    start = time.time()
    for step in range(1, args.stage2_steps + 1):
        ...
        interp = torch.zeros((), device=main.device)
        per = {}
        for section in base.SECTIONS:
            loss, _logits_b, _labels_b = base.decoder_forward(
                decoders[section],
            ...
            interp = interp + loss
            per[section] = float(loss.item())
        total = main if mode == "heima_baseline" else main + args.lambda_interp * interp
        total.backward()
        grad_a, finite = base.grad_norm(model_a.parameters())
        if not finite:
        ...
        if step == 1 or step == args.stage2_steps or step % args.log_every == 0:
            logs.append({"step": step, "ntp_loss": float(main.item()), "interp_loss": per, "total": float(total.item()), "grad_A": grad_a})
