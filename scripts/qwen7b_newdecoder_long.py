...
THINK_END = "<THINK_END>"
ANSWER = "<ANSWER>"
SPECIAL_TOKENS = [THINK, THINK_END, ANSWER]
EXPAND_PROMPT = "Please expand the latent token into textual information\n"
DEFAULT_MANIFEST = "/data2/zhouxiaoling/latent_cot/nocap_runs/LLaVA_COT_90K_NOCAP_STAGE1_90K_STAGE2_90K/manifest_train90k_eval5k_nocap.json"
DEFAULT_RUN = "/data2/zhouxiaoling/latent_cot/runs/QWEN7B_90K_NOCAP_LSYNC1_NEWDECODER_S1_35K_S2_25K_20260808"
SEED = 42
LR = 2e-4
PER_DEVICE_BATCH = 1
GRAD_ACCUM = 1
...
def load_split(path: Path) -> tuple[list[dict], list[dict], dict]:
    ...
    for rows in (train, eval_rows):
        for r in rows:
            r["raw_K"] = int(r.get("raw_K") or r.get("latent_count") or max(1, round(float(r.get("cot_token_count") or 2) * 0.5)))
...
def load_model_and_projector(trainable: bool = True):
    tok = load_tokenizer()
...
def main_sequence(tok, row: dict) -> tuple[list[int], dict[str, list[int] | int]]:
    ...
    seq = q + [think_id] * k + [end_id, answer_id] + ans
    q_len = len(q)
    think_positions = list(range(q_len, q_len + k))
    boundary_positions = [q_len + k, q_len + k + 1]
    answer_positions = list(range(q_len + k + 2, len(seq)))
    return seq, {
        "q_len": q_len,
        "k": k,
...
def run_audit_smoke(args):
    ...
    sample_info = {
        ...
        "answer_active_token_count": comps["answer_active_token_count"],
        "sync_active_token_count": comps["sync_active_token_count"],
        "decode_active_token_count": comps["decode"]["active_count"],
        "loss_answer_mean": float(comps["answer_loss"].detach().cpu()),
        "loss_sync_mean": float(comps["sync_loss"].detach().cpu()),
    ...
    passed = all(
        ...
        for x in [sync_model_grad, decode_z_grad, decode_model_grad, decode_projector_grad]
    )
    ...
    for step in range(1, args.smoke_steps + 1):
        ...
        loss = c["answer_loss"] + c["sync_loss"] + c["decode"]["loss"]
        ...
        rec = {
            ...
            "L_answer": float(c["answer_loss"].detach().cpu()),
            "L_sync": float(c["sync_loss"].detach().cpu()),
...
def validation_nll(tok, model, projector, rows: list[dict], n: int = 64, stage2: bool = False) -> dict:
    ...
    return {"n": len(vals), **{f"val_{k}": mean(k) for k in ["answer", "sync", "first_sync", "continuation_sync", "boundary", "answer_acc", "decode"]}}
...
def generation_protocol_audit(tok, model, rows: list[dict], n: int = 32, max_new_limit: int = 512) -> dict:
    ...
    for r in rows[:n]:
        ...
        with torch.no_grad():
            ...
            first_prob = float(logp[think_id].exp().cpu())
            first_rank = int((logits > logits[think_id]).sum().detach().cpu()) + 1
        ...
        results.append({
            ...
            "first_think_probability": first_prob,
            "first_think_rank": first_rank,
    ...
    return {
        ...
        "first_THINK_probability_mean": mean("first_think_probability"),
        "first_THINK_rank_mean": mean("first_think_rank"),
...
def run_stage1(args):
    ...
    cfg = {
        ...
        "loss": {"stage1": "L_answer + 1.0*L_sync", "stage2": "L_answer + L_sync + L_decode", "decode": "causal CE on gold CoT/EOS from expand_prompt + projected z prefix", "pooling_cosine_default": False},
        "batch": trainable_report(tok, model, projector),
        "stage1_target": args.stage1_steps,
        "stage1_extension_ceiling": 45000,
    ...
    t0 = now()
    checkpoint_steps = {1000, 5000, 10000, 20000, 25000, 30000, 35000, 40000, 45000}
    ...
    for step in range(1, args.stage1_steps + 1):
        ...
        loss = c["answer_loss"] + c["sync_loss"]
        if not torch.isfinite(loss):
        ...
        rec = {
            ...
            "first_sync_nll": float(c["first_sync_loss"].detach().cpu()),
            "continuation_sync_nll": float(c["continuation_sync_loss"].detach().cpu()),
            "K": c["meta"]["k"],
            "step_runtime": now() - s0,
            "elapsed": now() - t0,
            "peak_gb": torch.cuda.max_memory_allocated() / 1024**3,
        ...
        if step in checkpoint_steps:
            ...
            if step == 30000:
                ratio = ga.get("generated_THINK_over_gold_N_mean") or 0.0
                target_ratio = ga.get("generated_THINK_over_target_K_mean") or 0.0
                if (ga.get("generated_THINK_mean") or 0.0) <= 1.0:
                    stop_after_30k = True
                    write_json(status, {"status": "stopped_at_30k_protocol_fail", "reason": "free generation produced near-zero sync tokens", "step": step, "generation_protocol": ga})
                    break
                if ratio < 0.4 and target_ratio < 0.8:
                    # Continue to configured target; extension decision is left to later checkpoint metrics.
                    pass
...
def main():
    ...
    ap.add_argument("--mode", choices=["audit-smoke", "stage1"], required=True)
    ...
    ap.add_argument("--stage1-steps", type=int, default=35000)
